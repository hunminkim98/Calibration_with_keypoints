...
# camera directories
cam_dirs = [r'C:\Users\gns15\OneDrive\Desktop\Calibration_with_keypoints\demo_lod\json1',
...
# print(f"structure of paired_keypoints_list first element : {paired_keypoints_list[:2]}")

def unpack_keypoints(paired_keypoints_list):
    """
    Unpacks the keypoints from the paired_keypoints_list structure into two separate arrays
    for camera 1 and camera 2.
    
    Args:
        paired_keypoints_list: List of dictionaries containing paired keypoints
        
    Returns:
        points1: Array of keypoints from camera 1
        points2: Array of keypoints from camera 2
    """
    points1 = []
    points2 = []
    
    for frame_dict in paired_keypoints_list:
        for keypoint_idx, cameras in frame_dict.items():  # frame_dict의 items()를 순회
            points1.append(cameras['json1'])
            points2.append(cameras['json2'])
...
def compute_fundamental_matrix(paired_keypoints_list):
    ...
    F = cv2.findFundamentalMat(points1, points2, cv2.FM_RANSAC)
...
for j, K in enumerate(Ks):
    ...
    F = compute_fundamental_matrix(paired_keypoints_list)  # paired_keypoints를 paired_keypoints_list로 수정
    ...
    for _ in range(outer_iterations):
        ...
        for inner_iter in range(intrinsic_iterations):
            ...
            # loss = compute_intrinsic_optimization_loss([OPT_K[0, 0], OPT_K[1, 1]], points_3d_optimized, inlier2, R_optimized, t_optimized, u0, v0)
