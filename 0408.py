...
def optimize_bundle_adjustment(all_keypoints, P_all, max_iterations=100, tolerance=1e-4, initial_values=None):
    n_cameras = len(P_all)
    n_points = len(all_keypoints[0])
    cam_ids = [0] + [1] + [0] * (n_cameras - 2)
...
def run_bundle_adjustment(multi_cam_dir, confidence_threshold, Ks, all_best_results, max_iterations=100, tolerance=1e-4):
    ...
    print(f"Total keypoints: {sum(len(keypoints) for keypoints in all_keypoints)}")

    # Create projection matrices for all cameras
    P_all = create_P_all(all_best_results, Ks)
    print(f"Projection matrices: {P_all}")
